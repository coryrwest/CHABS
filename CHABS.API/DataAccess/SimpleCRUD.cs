...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
		...
		/// <summary>
		/// Parameterizes string where clause.
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <param name="connection"></param>
		/// <param name="conditions"></param>
		...
		//build where clause based on list of properties
		private static void BuildWhere(StringBuilder sb, IDictionary<string, object> props, object sourceEntity)
        {
			for (var i = 0; i < props.Count(); i++)
            {
                ...
                //the anonymous object used for search doesn't have the custom attributes attached to them so this allows us to build the correct where clause
                //by converting the model type to the database column name via the column attribute
				var propertyToUse = props.ElementAt(i);
                var sourceProperties = GetScaffoldableProperties(sourceEntity).ToArray();
                for (var x = 0; x < sourceProperties.Count(); x++)
                {
				...
				//sb.AppendFormat("{0} = @{1}", GetColumnName(propertyToUse), props.ElementAt(i).Value);
				if (i < props.Count() - 1)
                    sb.AppendFormat(" and ");
            }
        }
		...
		//Get all properties in an entity
		private static IDictionary<string, object> GetAllDynamicProperties(dynamic entity)
		{
			if (entity == null) entity = new { };
			return entity as IDictionary<string, object>;
		}
